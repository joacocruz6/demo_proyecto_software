...
const AfiliacionPrevisional = (props) => {
  ...
  if (useSpinner){
    make_query(props.resource + `?rut=${props.rut}`).then((jsonData) => {
      const keys = Object.keys(jsonData.data.GetAfiliacionPrevisional);
      const values = Object.values(jsonData.data.GetAfiliacionPrevisional);
      let newArr = [];
...
export default AfiliacionPrevisional;
